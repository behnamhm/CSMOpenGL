...
void Shader::CompileProgram() {
	...
	uniformAlbedo = glGetUniformLocation(shaderID, "albedo");
	uniformMetalic = glGetUniformLocation(shaderID, "metallic");
	uniformRoughness = glGetUniformLocation(shaderID, "roughness");
	uniformAo = glGetUniformLocation(shaderID, "ao");
	...
	unifromTexture_specular = glGetUniformLocation(shaderID, "texture_specular1");
...
void Shader::SetPointLights(std::vector<std::unique_ptr<PointLight>>& PointLightList, 
						unsigned int lightCount, unsigned int textureUnit, unsigned int offset)
{
	...
	{
		PointLightList[i]->UseLight(uniformPointLight[i].uniformAmbientIntensity, uniformPointLight[i].uniformColour,
